...
**Slide count.** Every slide has a `.pagenum` div hardcoded as `NN / 33`. The JS progress counter (`#progressTag`) computes its own total dynamically, but the per-slide `.pagenum` text does not — if a slide is added/removed, update all `.pagenum` strings to match the new total, **and** renumber every subsequent slide's `data-num` + `.pagenum` prefix (see the merge of the old glossary slides 05+06 for the sed/node approach used).

**Chrome show/hide.** The three top-right buttons (export PDF, text-scale, theme toggle) live inside `#chromeGroup`, a plain flex row toggled by the always-visible `#chromeToggle` button (`H` key, persisted to `localStorage`). Add any new top-chrome button as a child of `.chrome-group`, not as its own `position:fixed` element, so it hides/shows with the rest.

**Light-theme text contrast.** `--accent` (#ac8dff) is a background/fill color (`.panel.accent`, `mark.hi`, chart fills on the always-dark `.panel.ink`) — never use it as a foreground/text color directly. For text/icons colored "accent" on the normal (theme-flipping) panel surface, use `--accent-ink` instead: it equals `--accent` in dark theme but swaps to a darker, saturated violet (`#4018aa`) in light theme so it clears WCAG AA on light backgrounds. `.panel.ink` and `.panel.accent` locally redefine `--accent-ink`/`--muted` back to safe constants (see the CSS right after `.panel.ink{...}`), so *inside* those panels you can keep using the plain `--accent`/`--muted` inline as before — the scoped override handles it. When adding a new "signal-colored" text/icon element, default to `--accent-ink`, and only fall back to `--accent` when you're certain it sits inside `.panel.ink`.
